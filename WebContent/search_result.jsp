...
<%
	...
	{
		search=request.getParameter("search_location");
	}
	else if(radio.equals("salary"))
	{
		search=request.getParameter("search_salary");
	}
	else if(radio.equals("profile"))
	{
		search=request.getParameter("search_profile");
	}
	ArrayList<createjob_bean> jobs = new ArrayList<createjob_bean>();
	Dao obj = new Dao();
	jobs=obj.searchjob(radio,search);
...
<table>
	...
	<%
		int j=obj.check_applied(jobs.get(i).getId(),session.getAttribute("uname").toString());
		if(j>0)
		{
	...
	<tr>	
		<td colspan="3">
			<input type="submit" value="Apply" disabled> Already Applied..	
		</td>
	</tr>
	</table>
	<hr>
